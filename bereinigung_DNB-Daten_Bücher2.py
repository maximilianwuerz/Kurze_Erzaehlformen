...
df = pd.read_csv(
    r"C:\Users\ab32ihaq\FAUbox\Dissertation\FAU\Dissertation\Arbeitspakete\4_Feldanalyse\Relationale Datenbank\20250515\2008-2023_Gesamt_utf8.csv",
    sep=';', encoding='utf-8')
...
def bereinigen(df, bedingung, name):
    ...
    dazwischen.to_csv(f"bereinigte_{name}.csv", index=False, encoding='utf-8', sep=';')
    return df
...
def verteileaufrollen(df):
    ...
    sonstige_rollen = [
        ...
        "Verfasser eines Vorworts", "Verfasser eines Geleitworts", 
        "Verfasser eines Nachworts", "Mitwirkender", "Sonstige", 
        "Künstler", "Buchgestalter", "Zusammenstellender"
    ]
    
    for col in creator_col_names:
        df['Verfasser'] += df[col].apply(lambda x: f"{x.strip()}; " if pd.notna(x) and "Verfasser" in x and not any(phrase in x for phrase in ["Vorwort", "Nachwort", "Geleitswort"]) else "")
...
def bereinige_rollen_inhalte(content):
    zu_entfernende_woerter = [
        "Verfasser eines Vorworts", "Verfasser eines Geleitworts", "Verfasser eines Nachworts", 
        "Verfasser", "Herausgeber", "Sonstige", "Übersetzer", "Fotograf", 
        "Einbandgestalter", "Illustrator", "Mitwirkender", 
        "Künstler", "Buchgestalter", "Zusammenstellender"
    ]
...
df.to_csv(r'C:\Users\ab32ihaq\FAUbox\Dissertation\FAU\Dissertation\Arbeitspakete\4_Feldanalyse\Relationale Datenbank\20250515\2008-2023_Buch2.csv',
